...
export function bootstrapCards(startY: number): { currentCard: CardInstance | null; gates: Gate[] } {
  const frame = initializeCardStack(startY);
  currentCard = frame.currentCard;
  return { currentCard, gates: [...frame.gates] };
}

export function syncCards(spriteY: number): { currentCard: CardInstance | null; gates: Gate[] } {
