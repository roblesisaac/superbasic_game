...
function drawShaft(
  ...
  const highlightWidth = Math.max(2, Math.round(openingWidth * 0.18));
  const highlightLeft = Math.round(openingLeft + openingWidth / 2 - highlightWidth / 2);

  ctx.fillStyle = '#2b2b52';
  ctx.fillRect(highlightLeft, openingTop + 2, highlightWidth, Math.max(0, shaftBottom - openingTop - 4));
